...
class Seq_Database {
    ...
    void insert(string table, int num, vector<string> keys, vector<void*> records){
        unordered_map<string,unordered_map<string, void*>> database = *DB;
        try {
            DB->at(table);
        } catch (const std::out_of_range& oor) {
            unordered_map<string, void*>* x = new unordered_map<string, void*>();
            database[table] = *x;
    ...
    int remove(string table, int numCond, vector<pair<string,void*>> conditions){
        int count = 0;
    ...
    // Qry without sort
    vector<unordered_map<string,void*>> get(string table, vector<pair<string,void*>> conditions){
        int count = 0;
